...
% Load options
load('/rds/project/rds-6yHdsDfiMLk/MEG_objects/OscRSA_2016/MEG/Subjects/meg_0247/megdata/sensorlevel/MEGPLANARsource_pdabMefffbasic_0247.mat','option');
...
for sub = 1:length(optionz.subs)
    ...
    posterior = load([option.datadir option.sub_beg option.subs{sub} '/megdata/sens_post/' option.masknic{1} 'source_spatiotemporal_RDMs_60ms_sTW.mat']);
    frontal = load([option.datadir option.sub_beg option.subs{sub} '/megdata/sens_ant/' option.masknic{1} 'source_spatiotemporal_RDMs_60ms_sTW.mat']);
    ...
    outfile_ff = ([option.datadir option.sub_beg option.subs{sub} '/megdata/' 'connect_all/' option.masknic{1} mod_names{mod} 'ff.mat']); 
    outfile_fb = ([option.datadir option.sub_beg option.subs{sub} '/megdata/' 'connect_all/' option.masknic{1} mod_names{mod} 'fb.mat']);
...
outfile_ff_all = ([option.datadir 'all/' option.masknic{1} mod_names{mod} 'FF_all.mat']);
save(outfile_ff_all,'ff_all');

outfile_fb_all = ([option.datadir 'all/' option.masknic{1} mod_names{mod} 'FB_all.mat']);
...
outfile_fig = ([option.datadir 'figures/' option.masknic{1} mod_names{mod} 'FF_FB_all_sub.fig']);
savefig(outfile_fig)
