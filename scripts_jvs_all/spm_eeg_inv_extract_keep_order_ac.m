function [Ds] = spm_eeg_inv_extract_keep_order_ac(D,trialfront)
...
% Changed by AC to keep trials in same order as original file and retain
% rejection info. Also can specify larger time window than used for
% inversion
%
% Now can extract data from different file that was used for localisation
% (trial front)
%
% Updated to match spm12 r6906 AC Dec 16
...
clear D % don't need now we've extracted inv
Ds = [];

D = spm_eeg_load(trialfront);  % load alternative SPM file from which to get source timecourses for
...
try  % AC added to get anytime window
    It = inv.source.It;    
    It = find(abs(D.time - It(1)/1000) < 1/10000):find(abs(D.time - (It(2))/1000) < 1/10000); % get indices from ms bounds
catch
    It = inv.inverse.It;
end    
Np      = length(It);
