...
%       epoch = find(abs(D.time - -option.baseline/1000) < 1/10000):find(abs(D.time - (option.epoch_length-option.baseline)/1000) < 1/10000);
        ...
        if option.mnn
        % MNN/whitening using the baseline method (Guggenmos et al., 2018, NeuroImage)
        wDs = Ds(:,:,:);        
        wDs = permute(wDs,[3,1,2]); % conditions,space,time               
        btimes = find(Ds.time < (option.bl_ms(2))/1000 & Ds.time > (option.bl_ms(1)/1000))-1;
                
        sigma = cov1para(reshape(wDs(:,:,btimes),size(Ds,1),[])');                        
        sigma_inv = sigma^-0.5;
        for t = 1:size(wDs,3)
            wDs(:,:,t) = squeeze(wDs(:,:,t)) * sigma_inv;
        end
        Ds(:,:,:) = permute(wDs,[2 3 1]); clear wDs
        end % mnn
                ...
                elseif option.doPhasePower
                    ...
                    for i = 1:length(roi_data(:,1,1))
                        if option.zscore
                            ...
                            roi_data(i,:,:) = (squeeze(roi_data(i,:,:)) - repmat(squeeze(mean(roi_data(i,btimes,:),2)),[1 size(roi_data,2)])');
                        end
                    end
                end
