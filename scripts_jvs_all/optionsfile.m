...
option.s = s;
option.datadir = '/rds/project/rds-6yHdsDfiMLk/MEG_objects/OscRSA_2016/MEG/Subjects/';   % root of where to find subject data
% for RFX dir, see below
option.sub_beg = 'meg_';  % begining of subject folder name
option.subs = {'0233' '0247' '0248' '0251' '0255' '0256' '0258' '0259' '0261' '0262' '0263' '0264' '0265' '0266' '0274' '110056' '110087' '110098' '110126' '110319' '120276' '120469' '121317' '122172' '122405' '212153' '220115' '220335' '220518' '220828' '220974' '221740' '221828' '221977' '222125' '222367'};  % subjects to include
option.val = 4;  % SPM inversion to use
option.trialfront = 'pdabMefffbasic_'; % file to extract trial data from (can be different to above)
option.subdir1 = '/megdata/';  % subfolder where sub data is
option.subdir = [option.subdir1 'sensorlevel/'];  % folder you want extracted dta to be saved in
option.sub_out = [1 3 6 27 28];  % index of subjects to exclude from stats from list above


% File parameters reading EEGlab files and output filenames
option.front = 'pdabMefffbasic_';
...
if part == 0 || part == 1
% Model RDMs
%option.var_dir = '/rds/project/rds-6yHdsDfiMLk/MEG_objects/OscRSA_2016/MEG/RSA_ROI/RSA_models/'; % directory where RS models are saved
option.var_dir = 'C:\Users\Public\Data\MEG_object_naming\OscRSA_2016\MEG\RSA_ROI_Jacqueline\RSA_models\';
%option.models = ['RSA_sim_modelRDMs_DNN_comb_final'];
option.models = ['RSA_sim_modelRDMs_cornet_semfeats'];
...
option.mnn = 0;   % Apply multivariate noise normalisations based on baseline method
option.maskdir = '/rds/project/rds-6yHdsDfiMLk/MEG_objects/OscRSA_2016/MEG/ROIs/';
...
option.masknic = {'MEGPLANAR'}; % 'MEGMAG'
...
option.baseline_length = 1000;  % total length of the pre-stimulus period in ms (not including 0 ms)
...
% RSA settings
option.tw = round(40/option.srate); % size of sliding time-window (ms/samp_rate), cleanest if tw an even number
option.doavg = 0; % use averaged temporal signal over time window (spatial pattern, 1), or spatial-temporal pattern (0)
option.s = s;
option.rfxdir = ['/rds/project/rds-6yHdsDfiMLk/MEG_objects/OscRSA_2016/MEG/RSA_ROI/RSA_sim_modelRDMs_cornet_semfeats_cat_tw40_partial/']; % where to save RFX data
...
option.doPhase = 0;   % use phase instead of power
