function [clust_stats, clust_mass_perm, base_tmap] = permutation_cluster_test_2dtfr_func(data1,data2,fpmin,fpmax,tpmin,tpmax,perm_num,test,pval,rand_perms)
...
[~, clust_pos] = find_max_clust_mass(base_tmap,abs(tinv(pval,(size(data_1_rel,1)-1))));
[~, clust_neg] = find_max_clust_mass(-base_tmap,abs(tinv(pval,(size(data_1_rel,1)-1))));
clust_pos_data = [clust_pos clust_neg];
