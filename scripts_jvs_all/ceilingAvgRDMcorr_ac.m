function [ceiling_upperBound, ceiling_lowerBound, RDMcorrs_LOO, bestFitRDM]=ceilingAvgRDMcorr(refRDMestimates,RDMcorrelationType,monitor)
...
% AC adjusted to cope with NaNs in data RDMs
...
switch  RDMcorrelationType
    case 'Pearson'
        ...
        % single-subject RDMs.
        refRDMestimates=refRDMestimates-repmat(nanmean(refRDMestimates,2),[1 nDissims 1]);
        refRDMestimates=refRDMestimates./repmat(nanstd(refRDMestimates,[],2),[1 nDissims 1]);
        bestFitRDM=nanmean(refRDMestimates,3);
    ...
    case 'Spearman'
        ...
        % bjd33: there is a problem when there are NaNs in subject data, as
        % the sum of the ranks will not add up to the same value for each
        % subject. We need to "normalize" the tiedranks before calculating
        % the mean

        %refRDMestimates=reshape(tiedrank(reshape(refRDMestimates,[nDissims nSubjects])),[1 nDissims nSubjects]);

       % refRDMestimates=stRDMs; % testing
        refRDMestimates=squeeze(refRDMestimates);
        sumranks=nansum(refRDMestimates,1);
        truesumranks = nansum(tiedrank(1:size(refRDMestimates,1))); % assuming no nans
        normalizingfactor=truesumranks./sumranks;
        normalizingfactor = repmat(normalizingfactor,size(refRDMestimates,1),1);
        
        refRDMestimates = normalizingfactor.*refRDMestimates;
        refRDMestimates=reshape(refRDMestimates,[1 nDissims nSubjects]);
...
for subjectI = 1:nSubjects
    ...
    % remove any NaNs in ref and subject RDMs
    x = [cSubjectRDM; avgRefRDM_LOO];
    x(:,any(isnan(x),2)) = [];
    
    RDMcorrs_LOO(subjectI)=correlation(x(1,:),x(2,:),RDMcorrelationType);
end
...
for subjectI=1:nSubjects
    ...
    % remove any NaNs in ref and subject RDMs
    x = [cSubjectRDM; bestFitRDM];
    x(:,any(isnan(x),2)) = [];
        
    meanRDM_corrs(subjectI)=correlation(x(2,:),x(1,:),RDMcorrelationType);
end
...
% All the code after this is for Kendall Tau Method
...
function r=correlation(a,b,RDMcorrelationType)
    switch RDMcorrelationType
        ...
        otherwise
            % bjd33: need to account for NaNs
            %r=corr(a(:),b(:),'type',RDMcorrelationType);
            r=corr(a(:),b(:),'type',RDMcorrelationType, 'rows', 'pairwise');
            % end bjd33 edits
    end
